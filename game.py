...
def check_collisions(pipes):
    for pipe in pipes:
        if bird_rect.colliderect(pipe):
            death_sound.play().set_volume(0.09)
            return False
...
def score_display(game_state):
    ...
    if game_state == 'game_over':
        ...
        high_score_rect = score_surface.get_rect(center=(115, 420))
...
def pipe_score_check():
    ...
    if pipe_list:
        for pipe in pipe_list:
            if 70 < pipe.centerx < 80:
                score += 1
                score_sound.play().set_volume(0.09)
                break
...
while True:
    ...
    selected_character = menu.main(scr=screen, character=character)

    bird_frames = selected_character
    bird_index = 0
    bird_surface = bird_frames[bird_index]
    bird_rect = bird_surface.get_rect(center=(75, 256))
    ...
    game_over_surface = pygame.image.load('assets/msg.png').convert_alpha()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and game_active:
                    bird_movement = 0
                    bird_movement -= 6
                    flap_sound.play().set_volume(0.09)
                if event.key == pygame.K_SPACE and game_active == False:
                    game_active = True
                    pipe_list.clear()
                    bird_rect.center = (75, 256)
                    bird_movement = 0
                    score = 0
